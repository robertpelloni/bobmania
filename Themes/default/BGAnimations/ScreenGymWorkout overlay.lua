...
t[#t+1] = Def.ActorFrame {
    ...
    LoadFont("Common Normal") .. {
        Text = "BPM: --",
        InitCommand = function(self) self:y(40):zoom(1.0):diffuse(1,0,0,1) end,
        UpdateStatsCommand = function(self)
            if HRMAN then
                local bpm = HRMAN:GetHeartRate()
                self:settext("BPM: " .. bpm)
                -- Pulse if high
                if bpm > 130 then self:pulse():effectmagnitude(1.1, 1.0, 1.0)
                else self:stopeffect() end
            end
        end
    }
}
...
t[#t+1] = Def.ActorFrame {
    OnCommand = function(self)
        ...
        if DiscordRPC then
            DiscordRPC.SetPresence("Working Out", "Gym Mode")
        end
    end,

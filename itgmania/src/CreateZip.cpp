...
//   Rommel, Steve Salisbury, Dave Smith, Christian Spieler, Antoine Verheijen,
...
//       Such altered versions are further prohibited from misrepresentative use
...
typedef unsigned IPos;  // A Pos is an index in the character window. Pos is
...
typedef std::int64_t
    lutime_t;  // define it ourselves since we don't include time.h
...
typedef struct zlist {
  ush vem, ver, flg,
      how;  // See central header in zipfile.c for what vem..off are
  ulg tim, crc, siz, len;
...
/*
void __cdecl Trace(const char *x, ...) {va_list paramList; va_start(paramList,
x); paramList; va_end(paramList);} void __cdecl Tracec(bool ,const char *x, ...)
...
int putextended(struct zlist* z, WRITEFUNC wfunc, void* param) {
  // Write an extended local header described by *z to file *f. Returns a ZE_
  // code
  PUTLG(EXTLOCSIG, f);
  PUTLG(z->crc, f);
  PUTLG(z->siz, f);
...
int putcentral(struct zlist* z, WRITEFUNC wfunc, void* param) {
  ...
  if ((std::size_t)wfunc(param, z->iname, (unsigned int)z->nam) != z->nam ||
      (z->cext && (std::size_t)wfunc(param, z->cextra, (unsigned int)z->cext) !=
                      z->cext) ||
      (z->com &&
       (std::size_t)wfunc(param, z->comment, (unsigned int)z->com) != z->com)) {
    return ZE_TEMP;
  }
  return ZE_OK;
}
...
int putend(
    int n, ulg s, ulg c, std::size_t m, char* z, WRITEFUNC wfunc, void* param) {
...
class TZip {
 public:
  ...
  unsigned writ;         // how have we written. This is maintained by Add, not
...
ZRESULT TZip::open_dir() {
  ...
  csize = 0;
  ired = 0;
  attr = ZIP_ATTR_DIRECTORY2 | ZIP_ATTR_READABLE | ZIP_ATTR_WRITEABLE |
         ZIP_ATTR_DIRECTORY;
  isize = 0;
  return set_times();
}
...
void filetime2dosdatetime(
    const tm st, unsigned short* dosdate, unsigned short* dostime) {
  // date: bits 0-4 are day of month 1-31. Bits 5-8 are month 1..12. Bits 9-15
  // are year-1980 time: bits 0-4 are seconds/2, bits 5-10 are minute 0..59.
...
ZRESULT TZip::Add(const TCHAR* odstzn, const TCHAR* src, unsigned long flags) {
  ...
  // zip has its own notion of what its names should look like: i.e.
  // dir/file.stuff
  ...
  zfi.att = (ush)BINARY;
  zfi.vem =
      (ush)0xB17;     // 0xB00 is win32 os-code. 0x17 is 23 in decimal: zip 2.3
  zfi.ver = (ush)20;  // Needs PKUNZIP 2.0 to unzip it
  zfi.tim = timestamp;
  // Even though we write the header now, it will have to be rewritten, since we
  // don't know compressed size or crc.
  zfi.crc = 0;  // to be updated later
  zfi.flg = 8;  // 8 means 'there is an extra header'. Assume for the moment
                // that we need it.
  zfi.lflg = zfi.flg;     // to be updated later
  zfi.how = (ush)method;  // to be updated later
  zfi.siz =
      (ulg)(method == STORE && isize >= 0 ? isize : 0);  // to be updated later
  zfi.len = (ulg)(isize);                                // to be updated later
  zfi.dsk = 0;
  zfi.atx = attr;
  zfi.off =
      writ + ooffset;  // offset within file of the start of this local record
  // stuff the 'times' structure into zfi.extra

  // nb. apparently there's a problem with PocketPC CE(zip)->CE(unzip) fails.
  // And removing the following block fixes it up.
...
ZRESULT TZip::AddCentral() {
  ...
  for (TZipFileInfo* zfi = zfis; zfi != nullptr;) {
    ...
    writ += 4 + CENHEAD + (unsigned int)zfi->nam + (unsigned int)zfi->cext +
  ...
  if (okay) {
    int res = putend(
        numentries, center_size, pos_at_start_of_central + ooffset, 0, nullptr,
        swrite, this);
    if (res != ZE_OK) {
      okay = false;
    }
...
CreateZip::CreateZip() { hz = nullptr; }

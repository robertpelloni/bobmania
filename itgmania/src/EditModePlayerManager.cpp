...
void EditModePlayerManager::AddPlayers(const NoteData& note_data) {
	// In case of a routine chart, we only want PLAYER_1.
	// We check this upfront and force adding PLAYER_1, because FOREACH_EnabledPlayer
	// might skip PLAYER_1 if only PLAYER_2 is enabled (e.g. P2 is master).
    ...
    PlayerPlus& player = *players_[pn];
    player->Init(
        "Player", GAMESTATE->m_pPlayerState[pn], nullptr, nullptr, nullptr,
    ...
    player->SetX(THEME->GetMetricF(
        "ScreenGameplay",
        ssprintf(
            "PlayerP%d%sX", pn + 1, StyleTypeToString(style_type).c_str())));
...
void EditModePlayerManager::SetVisible(bool visible) {
	...
	if (GAMESTATE->GetCurrentStyle(PLAYER_1)->m_StyleType ==
		...
		if (players_.find(PLAYER_1) != players_.end()) {
...
bool EditModePlayerManager::HandleGameplayInput(
  ...
  PlayerNumber pn = input.pn;
	...
	if (gbt == GameButtonType_Step && players_.find(pn) != players_.end()) {
...
void EditModePlayerManager::SetupAutoplay() {
  for (auto& player : players_) {
    if (GAMESTATE->m_pPlayerState[player.first]
            ->m_PlayerOptions.GetCurrent()
            .m_fPlayerAutoPlay != 0) {
      GAMESTATE->m_pPlayerState[player.first]->m_PlayerController = PC_AUTOPLAY;
    } else {
      GAMESTATE->m_pPlayerState[player.first]->m_PlayerController =
...
void EditModePlayerManager::PlayTicks(GameplayAssist& gameplay_assist) {
  for (auto& player : players_) {
    ...
    gameplay_assist.PlayTicks(
        (*player.second)->GetNoteData(), (*player.second)->GetPlayerState());
    return;
  }
}
